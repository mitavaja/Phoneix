...
const BlockedUsers = () => {
  ...
  const fetchBlockedUsers = async () => {
    try {
      ...
      console.warn("Express API blocked users list unreachable. Using simulated fallback data.", error.message);
      setUsers([
        {
          _id: "blocked_usr_1",
          name: "Thomas Jefferson",
          email: "thomas@jefferson.org",
          role: "Buyer",
          companyName: "Monticello Imports",
          mobileNumber: "9876543211",
          gstType: "Non-GST",
          reason: "Compliance suspension due to documentation delay",
          createdAt: "2026-02-05T10:00:00Z",
          updatedAt: "2026-06-25T11:00:00Z",
          wallet: { availableBalance: 120.00, totalBalance: 500.00, holdBalance: 0 },
          orders: { total: 4, delivered: 3, pending: 0 }
        }
      ]);
    } finally {
      setLoading(false);
  ...
  const handleUnblock = async (id, name) => {
    try {
      await API.put(`/users/${id}/status`, { status: "Active" });
      alert(`Success: Account privileges reinstated for ${name}.`);
      fetchBlockedUsers();
      if (selectedUser && selectedUser._id === id) {
        setSelectedUser(null);
      }
    } catch (error) {
      alert(`Error unblocking account: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  return (
    <div className="space-y-8 select-none">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-extrabold text-[#0A1F44] tracking-tight">Blocked Users Register</h1>
          <p className="text-sm text-[#687280]">Review suspended/blocked user accounts, inspect violation reasons, and manage account reinstatement.</p>
        </div>
        <div className="glass-card px-4 py-2 rounded-xl border border-red-500/20 text-red-600 text-center font-bold text-sm bg-red-500/5">
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-[#687280]/20 text-[#687280] font-medium">
                ...
                <th className="pb-3 font-semibold">SUSPENSION REASON</th>
                <th className="pb-3 font-semibold">SUSPENDED ON</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan="9" className="py-8 text-center text-gray-500">
                    Loading blocked accounts registry...
                  </td>
                </tr>
              ) : (
                users.map((u) => (
                  <tr key={u._id} className="hover:bg-[#E5E7EB]/30 transition-colors">
                    ...
                    <td className="py-4">
                      ...
                      {u.companyName && <div className="text-[10px] text-gray-500 font-medium">{u.companyName}</div>}
                    </td>
                    ...
                    <td className="py-4 font-semibold text-red-600/80">{u.reason || "Suspended by Compliance Officer"}</td>
                    <td className="py-4 text-[#687280]">{new Date(u.updatedAt).toLocaleDateString()}</td>
              ...
              {!loading && users.length === 0 && (
                <tr>
                  <td colSpan="9" className="py-12 text-center text-gray-500 font-semibold">
                    🎉 Splendid! No suspended accounts on the platform.
                  </td>
                </tr>
              )}
            </tbody>
